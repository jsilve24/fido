...
collapsed model is given by: \$\$Y_j \sim \text{Multinomial}(\pi_j)\$\$
...
\text{Multinomial}(\pi_j)\$\$ \$\$\pi_j = \Phi^{-1}(\eta_j)\$\$ \$\$\eta
\sim MN\_{D-1 \times N}(\Lambda X, \Sigma, I_N)\$\$ \$\$\Lambda \sim
MN\_{D-1 x Q}(\Theta, \Sigma, \Gamma)\$\$ \$\$\Sigma \sim
InvWish(\upsilon, \Xi)\$\$ This function provides a means of sampling
from the posterior distribution of `Lambda` and `Sigma` given posterior
